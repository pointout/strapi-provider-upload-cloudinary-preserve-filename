...
module.exports = {
  ...
  init: config => {
    ...
    return {
      upload(file) {
        let filename = (config.preserve_filename == 'yes') ? file.name.replace(/\.[^/.]+$/, '') + '_' + file.hash + file.ext : file.hash + file.ext;
        let folder = (config.folder != '') ? config.folder.replace(/([^a-z0-9-]+)/gi, '') + '/' : '';
